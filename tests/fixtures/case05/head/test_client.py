...
def test_fetch_default_is_30s():
    ft = FakeTransport()
    Client(ft).fetch("http://x")
    # default 30000ms -> 30s at the transport layer
    assert ft.last["timeout_s"] == 30


def test_fetch_explicit_ms():
    ft = FakeTransport()
    Client(ft).fetch("http://x", timeout=1500)
    assert ft.last["timeout_s"] == 1.5
