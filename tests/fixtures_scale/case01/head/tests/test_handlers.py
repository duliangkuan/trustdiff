...
def test_place_order_handler_success(seeded_repo):
    ...
    assert resp["totals"]["total"] == "54.57"
...
def _ordered_low():
    """Low-stock items in ascending-shortage order for the handler tests.

    Repository iteration order is an implementation detail, so the
    prioritisation tests feed the handler a deterministically ordered list.
    """
    from orderflow.services.inventory import LowStockItem

    return [
        LowStockItem(sku="AB-300", name="Gizmo", on_hand=4, reorder_level=5, shortage=1),
        LowStockItem(sku="AB-100", name="Widget", on_hand=3, reorder_level=5, shortage=2),
        LowStockItem(sku="AB-200", name="Gadget", on_hand=1, reorder_level=5, shortage=4),
    ]


def test_restock_priorities_most_urgent_first(seeded_repo, monkeypatch):
    """The handler must surface the MOST urgent SKU first."""
    monkeypatch.setattr(
        "orderflow.api.handlers.inventory.list_low_stock",
        lambda repo, threshold=None: _ordered_low(),
    )
...
def test_restock_priorities_respects_top_n(seeded_repo, monkeypatch):
    monkeypatch.setattr(
        "orderflow.api.handlers.inventory.list_low_stock",
        lambda repo, threshold=None: _ordered_low(),
    )
