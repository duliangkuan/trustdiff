...
def proportional_shipping_fee(merchandise_subtotal: Decimal, rate: Decimal) -> Decimal:
    """Shipping fee charged as a fraction of the merchandise subtotal.

    Used for marketplace sellers who ship at a percentage of order value
    rather than a flat fee. The result is a two-place Decimal rounded
    HALF_UP — previously this multiplied through float and truncated, so a
    subtotal of 10.00 at 12.5% produced 1.25 but 10.05 produced 1.25 instead
    of the correct 1.26.
    """
    fee = merchandise_subtotal * rate
    return _money(fee)
