...
def test_notify_customers_survives_a_bad_send(seeded_repo, transport):
    """A flaky transport must not crash the notify handler; good recipients
    still go out."""
    transport.fail_for = {"bo@example.com"}
    resp = handlers.notify_customers_handler(
        seeded_repo, transport, ["C1", "C2", "C3"], "sale!"
    )
    # The handler returns cleanly and the healthy recipients were sent.
