...
def test_compute_tax_uses_config_rate():
    # VAT_RATE default 0.09
    assert pricing.compute_tax(Decimal("100.00")) == Decimal("9.00")
...
def test_price_order_standard_customer():
    ...
    assert bd.tax == Decimal("1.80")
    assert bd.total == Decimal("21.80")
...
def test_price_order_gold_discount():
    ...
    # taxable base after discount = 90, tax = 8.10
    assert bd.tax == Decimal("8.10")
    assert bd.total == Decimal("98.10")
...
def test_discount_per_unit_empty_cart_raises_value_error():
    # Empty cart must raise ValueError("empty cart"), not ZeroDivisionError.
    customer = Customer(id="C1", name="n", email="e@e.com", tier="gold")
    with pytest.raises(ValueError) as exc:
        pricing.discount_per_unit(_order(), customer)
    assert "empty cart" in str(exc.value)
