...
def monthly_revenue(repo: Repository) -> dict[str, str]:
    """Revenue grouped by calendar month, keyed by "YYYY-MM".

    Months are derived from each order's UTC `created_at` timestamp. Orders
    that do not count as revenue (cancelled, unless the flag is set) and orders
    with no created_at are skipped. Returns a {month: amount-string} dict whose
    amounts are two-place Decimal strings.
    """
    buckets: dict[str, Decimal] = {}
    for order in repo.list_orders():
        if not order.lines or not order.created_at:
            continue
        if not _counts_as_revenue(order):
            continue
        month = order.created_at[:7]  # "YYYY-MM" prefix of the ISO timestamp
        amount = order_service.compute_totals(repo, order).total
        buckets[month] = buckets.get(month, Decimal("0")) + amount
    return {month: str(_money(amt)) for month, amt in sorted(buckets.items())}
