...
def compute_tax(taxable_base: Decimal) -> Decimal:
    ...
    The rate is sourced from config.VAT_RATE (currently 6% following the
    2026 VAT policy change) so a future rate change is a one-line config edit.
    """
...
def discount_per_unit(order: Order, customer: Customer) -> Decimal:
    ...
    return money(total_discount / order.item_count())
